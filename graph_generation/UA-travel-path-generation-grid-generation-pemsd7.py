...
GRID_UNIT_METER = 3*MILE_TO_METER
    ...
    for i in tqdm.tqdm(range(len(grid_gdf))):
        for j in range(len(grid_gdf)):
            if i == j or grid_connectivity[i, j] == 0:
                continue
            if len(grid_idx_elems[i]) < 10 or len(grid_idx_elems[j]) < 10:
                continue
...
simcopy = sim_array.copy()
simcopy[simcopy < .9] = 0
plt.matshow(simcopy)
plt.matshow(new_adj_mx)
print('simcopy \t distance_graph_loaded', np.count_nonzero(simcopy))
