...
#          popup_click=[popup_element('osmidstr')]),
    ...
    Layer(osm_primary,  basic_style(color='black'), encode_data=False),
    Layer(osm_motorway,  basic_style(color='blue'), encode_data=False),
...
tmp_gdf_list = []
mcolor = ['blue', 'purple', 'green']
for k in amygraph_list:
...
Map(
    ...
    + [
        Layer(tmp_gdf, basic_style(color=mcolor[i])) for i, tmp_gdf in enumerate(tmp_gdf_list)
    ]
    + [
...
Map([
#          popup_click=[popup_element('osmidstr')]),,
    Layer(grid_gdf[grid_gdf['num_nodes'] > 0], basic_style(color='#FFFFFFFF', opacity=0, stroke_width=2, stroke_color='green')),
#     Layer(grid_gdf[grid_gdf['num_nodes'] > 0], color_continuous_style('num_nodes', opacity=.1), popup_click=popup_element('idx')),
    
    Layer(osm_nodes,  basic_style(color='black')),
    Layer(osm_edges,  basic_style(color='#bbbbbb'), encode_data=False),
    Layer(osm_secondary,  basic_style(color='#777777'), encode_data=False),
    Layer(osm_primary,  basic_style(color='black'), encode_data=False),
    Layer(osm_motorway,  basic_style(color='blue'), encode_data=False),
    Layer(gpd.GeoDataFrame(geometry=closest_line_list)),
